...
*   **沉浸式笔记**：基于 Bytemd 的 Markdown 编辑器，支持 Mermaid 图表、数学公式与代码高亮，写作体验行云流水。
...
*   **macOS**: 双击 \`.dmg\` 文件，将 Zero Snippets 图标拖入 Applications 文件夹。
...
2.  **搜索/新建**: 输入关键词查找现有片段，或点击右上角“+”号新建一个代码片段。
3.  **编辑**: 在编辑器中粘贴您的代码，选择语言（如 JavaScript），并添加描述标签。
4.  **保存**: 点击保存按钮，您的智慧结晶即被安全存入本地数据库。
5.  **复用**: 下次需要时，再次 \`F1\` 搜索，点击复制按钮，直接粘贴到您的 IDE 中。
